...
# LLM model (OpenRouter model ID) — có thể đổi sang model ":free" nếu chưa có credit
...
def reorder_for_llm(chunks: list[dict]) -> list[dict]:
    ...
    # front: index chẵn (0, 2, 4...) → đặt ở đầu
    # back:  index lẻ  (1, 3, 5...) → đảo ngược rồi đặt ở cuối
    front = chunks[::2]
    back = chunks[1::2]
    return front + back[::-1]
...
def generate_with_citation(query: str, top_k: int = TOP_K) -> dict:
    ...
    if not api_key:
        # Fallback khi không có API key (để test offline / unit test)
        answer = (
            "Tôi không thể xác minh thông tin này từ nguồn hiện có "
            "(thiếu OPENROUTER_API_KEY / OPENAI_API_KEY)."
        )
        return {
            "answer": answer,
            "sources": chunks,
